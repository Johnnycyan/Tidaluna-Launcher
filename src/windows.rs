...
/// Find the latest app-x.xx.x directory within a TIDAL/Discord-style install directory
/// and return the path to the executable within it.
pub fn get_latest_executable(dir: &std::path::Path) -> Result<std::path::PathBuf, String> {
    ...
    Err("Failed to find a valid TIDAL executable".into())
}
